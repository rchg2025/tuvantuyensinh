...
async function main() {
  ...
  try {
    ...
    const siteVerification = google.siteVerification({ version: 'v1', auth: jwtClient });
    
    // Insert verification
    const insertResponse = await siteVerification.webResource.insert({
      verificationMethod: 'FILE',
      requestBody: {
        site: {
          identifier: 'https://ts26.nsg.edu.vn/',
          type: 'SITE'
        }
      }
    });
